function history() {
  ...
  //
  //   $.ajax({
  //     url: "http://127.0.0.1:8000/student/outpassHistory",
  //     type: "GET",
  //     headers: {
  //       Authorization: "Bearer " + access_token
  //     },
  //     success: function(data) {
  //       var approved_outpasses = JSON.parse(data.approved_outpasses);
  //       console.log(approved_outpasses);
  //       var html = "";
  //       $("#history1").html(html);
  //       for (var i = 0; i < approved_outpasses.length; i++) {
  //         var from = approved_outpasses[i]["fromDate"];
  //
  //         var to = approved_outpasses[i]["toDate"];
  //         var id = approved_outpasses[i]["outpassId"];
  //         var purpose = approved_outpasses[i]["purposeOfLeave"];
  ...
  //           `">
  //             <h5>` +
  //           approved_outpasses[i]["student"]["user"] +
  //           `</h5>
  ...
  //           id +
  //           `">`;
  //         if (approved_outpasses[i]["approved_by_warden"] === false) {
  //           html += `
  //               <button class="remove" id="remove"><i class="fa fa-trash"></i></i><b>Delete</b></button>
  //               <button class="close1" id="close"><i class="fa fa-times"></i></button>`;
  //         } else {
  //           html += `
  //               <button class="generate11" id="generate11"><i class="fa fa-check"></i></i><b>Download Pdf</b></button>&nbsp;
  //               <button class="qr-c" id="qr-c"><i class="fa fa-qrcode"></i></i><b> QR Code</b></button>`;
  //         }
  //         html += `<button class="view" id="view"><i class="fa fa-eye"></i><b>View</b></button>
  //             </div>
  ...
  //             );
  //             $("#num_of_days1111").val(approved_outpasses[0]["no_of_days"]);
  ...
  //             $("#preview111").attr(
  //               "class",
  //               "js--image-preview js--no-default js--no-default"
  //             );
  //             //console.log("background-image: url('"+outpassDetails[0]['whatsappImg']+"')")
  //             $("#preview111").attr(
  //               "style",
  //               "background-image: url('http://127.0.0.1:8000" +
  //                 outpassDetails[0]["whatsappImg"] +
  //                 "')"
  //             );
  //             $("#load").hide();
  //             $("#main1111").removeClass("blur");
  ...
  //         if (
  //           confirm(
  //             "Are You Sure You Want To Delete The Outpass Request Permanently?"
  //           )
  //         ) {
  ...
  //           success: function(data) {
  //             if (data.status === "200") {
  //               alert("Successfully Deleted Pending Outpass!!");
  //               $("#card-" + id)
  //                 .next()
  //                 .remove();
  //               $("#card-" + id)
